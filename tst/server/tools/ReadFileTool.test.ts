			...
			expect(readToolDefinition.inputSchema).toEqual({
				...
				properties: {
					...
					reasoning: {
						type: 'string',
						description:
							'A brief explanation (1-2 sentences) of why you need to read this file and how it will help accomplish the task.',
					},
				},
		...
		describe('async behavior', () => {
			it('should handle async file reading correctly', async () => {
				...
				mockReadFile.mockImplementation(
					() =>
						new Promise(resolve => setTimeout(() => resolve(fileContent), 10)),
				);
