...
export const Main: React.FC<MainProps> = ({agent, task}) => {
	...
	const [focusedSection, setFocusedSection] = useState<FocusSection>('eventStream');
	...
	const bodyHeight = Math.max(screenDimensions[1] - headerHeight - footerHeight - topMargin - bottomMargin, 0)

	return (
		<Box flexDirection="column" flexGrow={1} height={screenDimensions[1]} width={screenDimensions[0]} overflow="hidden">
			{/* Header */}
			<Box borderStyle="round" borderDimColor flexShrink={0} height={headerHeight} marginTop={topMargin} paddingX={1}>
				<Box flexDirection="column">
					<Text bold color={colors.accentColor}>OrgChart - {agent.name}</Text>
					<Text color={colors.subtextColor}>Working Directory: {currentDir}</Text>
					<Text color={colors.subtextColor}>Total Cost: ${totalCost.toFixed(2)}</Text>
				</Box>
			</Box>
			...
			<Box flexGrow={1} gap={0.5} overflow="hidden">
				...
				<Box
					...
					flexShrink={1}
					borderDimColor
					borderColor={focusedSection === 'agentTree' ? colors.highlightColor : colors.subtextColor}
				...
				<Box 
					...
					borderStyle="round" 
					borderDimColor
					borderColor={focusedSection === 'eventStream' ? colors.highlightColor : colors.subtextColor}
					flexDirection="column" 
					height={bodyHeight} 
					overflow="hidden"
				>
					<EventStream events={events} focused={focusedSection === 'eventStream'} height={bodyHeight-2}/>
				</Box>
			</Box>
