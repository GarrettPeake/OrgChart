...
export class LLMProvider {
    ...
    constructor(apiKey?: string, baseUrl: string = 'https://openrouter.ai/api/v1') {
        ...
        if (!key) {
            const error = new Error('OpenRouter API key is required. Set OPENROUTER_API_KEY environment variable.');
            Logger.error(error, "LLMProvider initialization failed - missing API key");
            throw error;
        }
    ...
    async chatCompletion(request: ChatCompletionRequest, tools: ToolDefinition[]) {
        let retries = 0
