...
export const bashToolDefinition: ToolDefinition = {
	...
	enact: async (args: {command: string, requires_approval: boolean}): Promise<string> => runCommandSafely(args.command),
	formatEvent: async (args: {command: string, requires_approval: boolean}): Promise<StreamEvent> => ({title: `Bash(${args.command})`, content: ""}),
}

function runCommandSafely(command: string): string {
	Logger.info(`Executing command: ${command}`)
	if(command.includes("rm -rf") || command.includes("~") || command.includes("shutdown")) {
