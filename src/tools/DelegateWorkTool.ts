    ...
    inputSchema: {
        ...
        properties: {
            ...
            agentId: {
                ...
                enum: Object.entries(agents).map(([k, v]) => v.level < level ? k : undefined).filter(s => s !== undefined)
            },
        },
        required: ["task", "agentId"],
    },
    enact: async (args: {agentId: string, task: string}): Promise<string> => "Handled in Task.ts",
    formatEvent: async (args: {agentId: string, task: string}): Promise<StreamEvent> => ({title: `Spawn Agent(${agents[args.agentId]})`, content: args.task}),
})