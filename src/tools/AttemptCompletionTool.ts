...
export const attemptCompletionToolDefinition: ToolDefinition = {
	...
	enact: async (args: {result: string}): Promise<string> => "Handled in Task.ts",
	formatEvent: async (args: {result: string}): Promise<StreamEvent> => ({title: "Task Complete", content: args.result}),
}