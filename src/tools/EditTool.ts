...
export const editToolDefinition: ToolDefinition = {
	...
	enact: async (args: {file_path: string, edits: {old_string: string, new_string: string}[]}): Promise<string> => "NOT IMPLEMENTED",
	formatEvent: async (args: {file_path: string, edits: {old_string: string, new_string: string}[]}): Promise<StreamEvent> => ({title: `Edit File(${args.file_path})`, content: Object.entries(args.edits).map((e) => `SEARCH: ${e[0]}\nREPLACE: ${e[1]}`).join("\n\n")}),
}