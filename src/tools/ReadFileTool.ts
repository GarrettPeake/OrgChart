...
export const readToolDefinition: ToolDefinition = {
    ...
    enact: async (args: { file_path: string }): Promise<string> => await readFormatted(args.file_path),
    formatEvent: async (args: { file_path: string }): Promise<StreamEvent> => ({title: `Read(${args.file_path})`, content: await readFormattedTruncated(args.file_path)}),
}
...
const readFile = async (file_path: string): Promise<string> => {
    ...
    try {
        ...
        throw new Error(`Failed to read file ${file_path}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
};